...
_not_ the values. The actual values (the latest version) of each secret will be fetched
...
The **NuGet Tool Installer** puts you in control of the version of NuGet used by both the **NuGet Restore** and **NuGet Command** tasks. By default, the tasks will use a well-known, tested version. If you want to override that, simply add a **NuGet Tool Installer** task before your other NuGet build tasks.

**NuGet Restore** is now optimized for the most common scenario of restoring packages before a Visual Studio Build task. It also has better support for small projects that share a single NuGet feed: you can now pick an Azure Artifacts feed and have it added to an auto-generated NuGet.Config.
